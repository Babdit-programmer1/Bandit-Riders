...
const RiderDashboard: React.FC = () => {
  ...
  const updateStatus = (id: string, newStatus: DeliveryStatus) => {
    ...
    switch(newStatus) {
      ...
      /* Fix: Handle progress for the new IN_PROGRESS status used in simplified workflows */
        ...
        <div className="lg:col-span-4 space-y-8">
          <div className="bg-white p-10 rounded-[3.5rem] shadow-premium border border-slate-100 relative overflow-hidden">
            ...
            <div className="relative z-10 pt-4 text-center">
              <img src={user?.avatar} className="w-28 h-28 rounded-[2.5rem] border-4 border-white shadow-2xl mx-auto mb-6" alt="Profile" />
              ...
              <div onClick={() => { setIsOnline(!isOnline); authService.updateUser({ isAvailable: !isOnline }); }} className={`flex items-center justify-between p-6 rounded-3xl border-2 cursor-pointer transition-all ${isOnline ? 'bg-emerald-50 border-emerald-500 text-emerald-700' : 'bg-slate-100 border-slate-200 text-slate-500'}`}>
                <span className="text-xs font-black uppercase tracking-widest">{isOnline ? 'Dispatch Active' : 'Offline'}</span>
          ...
          <div className="bg-slate-900 p-10 rounded-[3.5rem] text-white shadow-3xl">
            <p className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.3em] mb-4">Total Earnings</p>
                  ...
                  <div className="flex flex-col lg:flex-row gap-10">
                    <div className="flex-1 space-y-4">
                      <div className="flex justify-between">
                        <span className="text-[10px] font-black text-indigo-500 uppercase tracking-widest">{job.id}</span>
                        <StatusBadge type="status" value={job.status} />
                      </div>
                      <h4 className="text-3xl font-black text-slate-900 tracking-tighter">{job.address}</h4>
                      <p className="text-xs font-bold text-slate-400">Pickup: {job.pickupAddress}</p>
                    </div>
                    <div className="lg:w-64 bg-slate-50 p-8 rounded-[2.5rem] text-center border border-slate-100 flex flex-col justify-center">
                      ...
                      <p className="text-3xl font-black text-slate-900 mb-4">{formatNaira(job.price)}</p>
                      <div className="space-y-2">
                        {job.status === DeliveryStatus.PENDING && <Button fullWidth onClick={() => updateStatus(job.id, DeliveryStatus.ACCEPTED)} className="py-4 bg-indigo-600">Accept Mission</Button>}
                        {job.status === DeliveryStatus.ACCEPTED && <Button fullWidth onClick={() => updateStatus(job.id, DeliveryStatus.PICKED_UP)} className="py-4 bg-purple-600">Arrived</Button>}
                        {job.status === DeliveryStatus.PICKED_UP && <Button fullWidth onClick={() => updateStatus(job.id, DeliveryStatus.IN_TRANSIT)} className="py-4 bg-amber-500">Transit</Button>}
                        {job.status === DeliveryStatus.IN_TRANSIT && <Button fullWidth onClick={() => updateStatus(job.id, DeliveryStatus.DELIVERED)} className="py-4 bg-emerald-600">Deliver</Button>}
                      </div>
                    </div>
                  </div>
                </div>
